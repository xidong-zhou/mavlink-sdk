...
#define MAVLINK_MSG_ID_PING_LEN 13
#define MAVLINK_MSG_ID_PING_MIN_LEN 13
#define MAVLINK_MSG_ID_2_LEN 13
#define MAVLINK_MSG_ID_2_MIN_LEN 13

#define MAVLINK_MSG_ID_PING_CRC 100
#define MAVLINK_MSG_ID_2_CRC 100
...
{
    return mavlink_msg_ping_pack(system_id, component_id, msg, ping->time_usec, ping->seq, ping->target_system);
}
...
{
    return mavlink_msg_ping_pack_chan(system_id, component_id, chan, msg, ping->time_usec, ping->seq, ping->target_system);
}
...
static inline void mavlink_msg_ping_send(mavlink_channel_t chan, uint64_t time_usec, uint32_t seq, uint8_t target_system)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_PING_LEN];
    _mav_put_uint64_t(buf, 0, time_usec);
...
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    mavlink_msg_ping_send(chan, ping->time_usec, ping->seq, ping->target_system);
#else
 ...
 */
static inline void mavlink_msg_ping_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  uint64_t time_usec, uint32_t seq, uint8_t target_system)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
